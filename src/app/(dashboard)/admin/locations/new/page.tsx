...
export default async function NewLocationPage() {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6 pb-12">
      ...
      >
        <ArrowLeft className="h-4 w-4" /> Back to Route Terminals
      </Link>
      ...
      <div className="bg-white dark:bg-[#0e131f] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-8 shadow-xl">
        <div className="flex items-center gap-3 mb-6 pb-6 border-b border-slate-200 dark:border-slate-800/80">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              Add Shuttle Route Terminal
            </h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Register a new pickup or drop-off location hub.
            </p>
          </div>
        </div>
        ...
        <form action={handleCreate} className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Terminal Location Name</label>
            <input
              type="text"
              name="name"
              required
