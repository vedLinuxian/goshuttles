...
export default function ErrorPage({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-[40vh] flex-col items-center justify-center gap-4 p-8 text-center" role="alert">
      <h1 className="text-lg font-semibold text-[var(--foreground)]">Passenger workspace unavailable</h1>
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/passenger/dashboard" className={buttonVariants({ variant: "secondary", size: "sm" })}>Back to dashboard</Link>
      </div>
    </div>
  );
}