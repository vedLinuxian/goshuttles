...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { variant?: "default" | "glass" | "amber" }>(
  ({ className, variant = "default", ...props }, ref) => (
    <div ref={ref} className={cn(
      "rounded-2xl border border-[var(--border)] text-[var(--card-foreground)] transition-all duration-200",
      variant === "default" && "bg-[var(--card)] shadow-sm dark:shadow-xl dark:shadow-black/20",
      variant === "glass" && "bg-[var(--card)]/90 shadow-lg backdrop-blur-xl dark:shadow-2xl",
      variant === "amber" && "border-amber-500/30 bg-amber-500/10 shadow-sm dark:shadow-lg",
      className,
    )} {...props} />
  ),
);
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => <p ref={ref} className={cn("text-sm leading-relaxed text-slate-500 dark:text-slate-400", className)} {...props} />);
